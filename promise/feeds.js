(function Feeds(){
	...
	function setupFeeds() {
		...
		(function listenToFeedStatus(){
			...
			Promise.race([slow_feed_toggle,fast_feed_toggle])
			.then(function then(){
				monitorFeedStatus(st1,st2);
			})
			// listen again
			.then(listenToFeedStatus);
		})();
	...
	function setupFeed(feedID,delay,defineTogglePromiseChain) {
		...
		function setupStreamEventHandler(pr) {
			Promise.all([pr,feed_status])
			.then(function pluck(args){
	...
	function setupMyFeed() {
		...
		fromEvent("item-selection",function setupItemSelection(pr){
			pr.then(handleItemSelection);
		});
	...
	function fromEvent(evtName,definePromiseChain) {
		EVT.on(
			evtName,
			promiseBasedEventHandler(definePromiseChain)
		);
	}
